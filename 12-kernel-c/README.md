...
请参考os-guide.pdf进行解释:
为什么`pointer .c`的反汇编不像你所期望的那样? 字符"Hello"的ASCII码"0x48656c6c6f"被保存在哪里？
常量单独存放在一个内存区域里面，然后使用内存地址进行取值，之所以反汇编找不到"Hello"是因为反汇编器区分不开数据和指令，所以结尾会出现很奇怪的`dec eax`指令等，其实看数据就知道这是"Hello"的数据，这个部分也不会被执行到，因为早已执行`ret`返回了。
